...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements SysUserService {
	 ...
	 */
	@Override
	public Result<UserInfo> getUserInfo(UserDTO query) {
		UserVO dbUser = baseMapper.getUser(query);

		if (dbUser == null) {
			return Result.failed(MsgUtils.getMessage(ErrorCodes.SYS_USER_USERINFO_EMPTY, query.getUsername()));
		}
	 ...
	 */
	@Override
	public IPage<UserDTO> getUsersWithRolePage(Page<UserDTO> page, UserDTO userDTO) {
	...
	public Result<Boolean> updateUserInfo(UserDTO userDto) {
		...
		sysUser.setUserId(Objects.requireNonNull(SecurityUtils.getUser()).getId());
	 ...
	 */
	@Override
	public Result<Object> importUsers(List<UserExcelVO> excelVOList, BindingResult bindingResult) {
		// 通用校验获取失败的数据
	...
	@Override
	@Transactional(rollbackFor = Exception.class)
	public Result<Boolean> registerUser(RegisterUserDTO userDto) {
        // 判断用户名是否已存在
	...
	public Result<Boolean> lockUser(String username) {
	...
	@Override
	@CacheEvict(value = CacheConstants.USER_DETAILS, key = "#userDto.username")
	public Result<Void> changePassword(UserDTO userDto) {
		SysUser sysUser = baseMapper.selectById(Objects.requireNonNull(SecurityUtils.getUser()).getId());
		...
		if (!ENCODER.matches(userDto.getPassword(), sysUser.getPassword())) {
			...
			return Result.failed(MsgUtils.getMessage(ErrorCodes.SYS_USER_UPDATE_PASSWORDERROR));
		}

		if (CharSequenceUtil.isEmpty(userDto.getNewpassword1())) {
	 ...
	 */
	@Override
	public Result<Void> checkPassword(String password) {
		SysUser sysUser = baseMapper.selectById(Objects.requireNonNull(SecurityUtils.getUser()).getId());
